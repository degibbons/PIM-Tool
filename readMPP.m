%% readMPP extracts the MPP data frame in the data file
function [MPPmat,finalIndex] = readMPP(filecontent,inIndex,x1,x2,y1,y2)
MPPmat = zeros(95,64);
fileLength = length(filecontent);
i = inIndex;
while i < fileLength-2 %While the end of the file has not been reached
    if filecontent(i) == 77 && filecontent(i+1) == 80 && filecontent(i+2) == 80 %Check if 'MPP' has been reached
    ...
    i = i + 1; %If 'MPP' has not been reached, increment the index until it is.
end
